...
function changeTask(event) {
  ...
  createConfirmChange.setAttribute(
    "onclick",
  ...
  createCancelChange.setAttribute(
    "onclick",
...
function confirmChange(changedTaskIdNum) {
  let changedInputField = document.getElementById(
    "changeInput" + changedTaskIdNum
  );
  if (changedInputField.value.length < 1) {
    ...
    tasks.forEach((element, index) => {
      if (index === changedTaskIdNum) {
        ...
        let parentElementForTask = document.getElementById(
          "task" + changedTaskIdNum
        );
        let taskName = parentElementForTask.firstChild.nextSibling;
        taskName.remove();
        let buttons = document.getElementById("buttons" + changedTaskIdNum);
